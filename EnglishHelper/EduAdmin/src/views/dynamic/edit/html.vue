<template>
  <div class="form-container">
    <div class="dynamic-container">
      <el-input v-model="dynamic.title" placeholder="请输入标题" style="width: 540px" />
      <el-input placeholder="请输入主题" v-model="dynamic.subject" style="width: 120px"/>
      <el-button @click="editDynamic">发布</el-button>
    </div>
...
<script>
import { getDyContent, updateDyContent } from '@/api/dynamic'
...
export default {
  ...
  mounted() {
    this.dynamic.id = this.$route.params.dynamicId
    this.getDynamicById(this.dynamic.id)
  },
  methods: {
    ...
    getDynamicById: function(id) {
      getDyContent(id).then(res => {
        if (res.code === 200) {
          if (res.response === undefined) {
            ...
            this.dynamic.title = res.response.title
            this.dynamic.subject = res.response.subject
          }
        } else {
