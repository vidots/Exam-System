<template>
  <div class="form-container">
    <div class="news-container">
      <el-input v-model="news.title" placeholder="请输入文章标题" style="width: 540px" />
      <el-button @click="editNews">发布</el-button>
    </div>
      ...
      getNews(id).then(res => {
        if (res.code === 200) {
          if (res.response === undefined) {
            ...
            this.news.title = res.response.title
            this.news.summary = res.response.summary
            this.news.format = res.response.format
          }
        } else {
